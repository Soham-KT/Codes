...
--Query: QueryNo 6
--todo: List out employee_id,last name,department id for all employees and 
--todo: rename employee id as “ID of the employee”, last name as “Name of 
--todo: the employee”, department id as “department ID”
USE assignment;
...
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Department_ID,Salary,Job_ID 
    FROM EMPLOY WHERE Salary between 3000 and 4500;

--Query: QueryNo 11
--todo: List out the employees who are working in department 10 or 20
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Department_ID,Job_ID 
...
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Department_ID,Job_ID
    FROM EMPLOY WHERE Last_Name like "S%H";
...
--Query: QueryNo 16
--todo:  List out the employees who are working in department
--todo: 10 and draw the salaries more than 3500
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Department_ID,Salary 
    FROM EMPLOY WHERE Department_ID=10 and Salary>3500;

--Query: QueryNo 17
--todo: list out the employees who are not receiving commission
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Department_ID,
    Job_ID FROM EMPLOY WHERE Comm is NULL;
...
--Query: QueryNo 20
--todo: List out the employee details according to their last_name
--todo: in ascending order and salaries in descending order 
USE assignment;
SELECT Employ_ID,Last_Name,First_Name,Middle_Name,Job_ID,Manager_ID,
    DATE_FORMAT(Hier_Date,"%d-%b-%y") as Hier_Date,Salary,Comm,
        Department_ID FROM EMPLOY 
            ORDER BY Last_Name ASC ,Salary DESC;

--Query: QueryNo 21
--todo: List out the employee details according to their last_name in
--todo: ascending order and then on department_id in descending order
USE assignment;
SELECT Employ_ID,Last_Name,First_Name,Middle_Name,Job_ID,Manager_ID,
    DATE_FORMAT(Hier_Date,"%d-%b-%y") as Hier_Date,Salary,Comm,
        Department_ID FROM EMPLOY 
...
SELECT DATE_FORMAT(Hier_Date,"%m") as Hier_Month,
    COUNT(Employ_ID)as No_Of_Employees FROM EMPLOY
        GROUP BY Hier_Month ORDER BY Hier_Month ASC;

--Query: QueryNo 26
--todo: List out the no.of employees for each month and year, in the ascending
--todo: order based on the year, month
USE assignment;
SELECT DATE_FORMAT(Hier_Date,"%Y")as Year,DATE_FORMAT(Hier_Date,"%m")as Month,
    count(*) as No_Of_Employees FROM EMPLOY GROUP BY Year,Month ORDER BY
        Year ASC, Month ASC;
...
use assignment;
select DATE_FORMAT(Hier_Date, "%D") from EMPLOY LIMIT 2;
...
USE assignment;
select Department_ID,Hier_Date,count(*) from EMPLOY group by Department_ID
    having DATE_FORMAT(Hier_Date,"%M %Y") = "April 1985" and count(*)>=3;
...
--Query: QueryNo 35
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Department_ID,Job_ID FROM EMPLOY 
...
SELECT * FROM EMPLOY WHERE Department_ID =
    (SELECT Department_ID FROM DEPARTMENT WHERE Location_ID =
        (SELECT Location_ID FROM LOCATION WHERE Regional_Group 
            = "NEW YORK"));
...
--Query: QueryNo 39
USE assignment;
UPDATE EMPLOY SET Salary = Salary /10 WHERE Job_ID =
    (SELECT Job_ID FROM JOB WHERE Function_ = "CLERK");
...
--Query: QueryNo 41
USE assignment;
SELECT First_Name,Last_Name,Employ_ID,Salary FROM EMPLOY WHERE Salary = (SELECT 
...
USE assignment;
SELECT First_Name,Employ_ID,Salary FROM EMPLOY as Emp1 WHERE (N-1)
    =(SELECT COUNT(DISTINCT(Emp2.Salary))FROM EMPLOY as Emp2 WHERE 
        Emp2.Salary > Emp1.Salary);

--Query: QueryNo 43
USE assignment;
SELECT First_Name,Employ_ID,Salary FROM EMPLOY WHERE Salary > ALL(SELECT 
...
-- Query: QueryNo 47 
USE assignment;
SELECT e.Employ_ID,e.First_Name,e.Department_ID,e.Salary FROM EMPLOY as e WHERE Salary 
    >(SELECT AVG(Salary) FROM EMPLOY WHERE Department_ID=e.Department_ID);

--Query: QueryNo 48
USE assignment;
SELECT EMPLOY.Employ_ID,EMPLOY.First_Name,EMPLOY.Last_Name,DEPARTMENT.Department_ID,
...
--Query: QueryNo 51
USE assignment;
SELECT d.Department_ID,d.Name,COUNT(*) as No_Of_Employees FROM DEPARTMENT as d 
...
SELECT d.Department_ID ,d.Name,COUNT(*) as No_Of_Employees FROM DEPARTMENT as d 
    INNER JOIN EMPLOY as e ON d.Department_ID=e.Department_ID GROUP BY d.Department_ID
        HAVING No_Of_Employees>=5 ORDER BY d.Name ASC;

--Query: QueryNo 54
USE assignment;
SELECT j.Job_ID,j.Function_ as Designation,COUNT(*) as No_Of_Employees FROM
...
SELECT l.Location_ID,l.Regional_Group,COUNT(*) FROM ((DEPARTMENT INNER JOIN 
    EMPLOY USING(Department_ID))INNER JOIN LOCATION as l USING(Location_ID))
        GROUP BY Location_ID HAVING l.Regional_Group = "NEW YORK";

--Query: QueryNo 56 
USE assignment;
SELECT e.First_Name,e.Employ_ID,e.Salary,s.Salary_Grade FROM EMPLOY as e
    INNER JOIN SALARYGRADES as s WHERE e.Salary BETWEEN s.Salary_MIN AND s.Salary_MAX
        ORDER BY s.Salary_Grade;

--Query: QueryNo 57 
USE assignment;
SELECT s.Salary_Grade,count(*) as No_Of_Employees FROM  EMPLOY as e 
    INNER JOIN SALARYGRADES as s WHERE e.Salary BETWEEN s.Salary_MIN AND s.Salary_MAX
...
--Query: QueryNo 59
USE assignment;
SELECT e.First_Name as "Employ",e.Employ_ID,m.First_Name as "Manager",m.Employ_ID as 
    "Manager_ID" FROM EMPLOY as e JOIN EMPLOY as m ON e.Manager_ID= m.Employ_ID;

--Query: QueryNo 60
USE assignment;
SELECT e.First_Name as "Employ",e.Employ_ID,e.Salary,m.First_Name as "Manager",m.Salary as 
    "Manager_Salary" FROM EMPLOY as e JOIN EMPLOY as m ON m.Salary < e.Salary;
    -- AND m.Salary < e.Salary;

--Query: QueryNo 61
USE assignment;
SELECT m.First_Name as Manager,m.Employ_ID as Manager_ID,COUNT(e.Employ_ID)as 
    No_Of_Employees FROM EMPLOY as e JOIN EMPLOY as m ON e.Manager_ID= m.Employ_ID
        GROUP BY Manager_ID;

--Query: QueryNo 62
USE assignment;
SELECT d.Department_ID,d.Name as Department_Name,Location_ID,Employ_ID,
    	First_Name,Job_ID FROM EMPLOY as e RIGHT JOIN DEPARTMENT as d 
            USING(Department_ID);

--Query: QueryNo 63
USE assignment;
SELECT First_Name,d.Department_ID,d.Name FROM EMPLOY as e RIGHT JOIN DEPARTMENT 
    as d USING(Department_ID) WHERE d.Department_ID in(SELECT Department_ID FROM 
        DEPARTMENT WHERE Name ="SALES" OR Name ="OPERATIONS");
...
select Function_ from JOB where Job_ID in
(select Job_ID from EMPLOY where Department_ID = 
(select Department_ID from DEPARTMENT where Name = "RESEARCH")) intersect
select Function_ from JOB where Job_ID in
(select Job_ID from EMPLOY where Department_ID = 
(select Department_ID from DEPARTMENT where Name = "ACCOUNTING")) 
order by Function_;